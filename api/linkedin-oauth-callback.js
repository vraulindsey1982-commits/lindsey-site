import { exchangeLinkedInCode } from './_lib/linkedin.js';
...
export default async function handler(req, res) {
  ...
  try {
    const { accessToken, personUrn } = await exchangeLinkedInCode(code);
    return res.status(200).json({ success: true, accessToken, personUrn });
  } catch (err) {
    console.error('Erreur linkedin-oauth-callback', err);
